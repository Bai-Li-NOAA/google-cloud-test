subdir <- "SS"
casedir <- file.path(maindir, case1$case_name)
input_filename <- em_input_filenames[1,]$SS

if(!("r4ss" %in% installed.packages()[,"Package"])) install.packages("r4ss")
library(r4ss)

dir.create(file.path(casedir, "output", subdir))
...
foreach (om_sim = 1:om_sim_num) %dopar% {
  ...
  file.copy(file.path(maindir, "em_input", "ss"), file.path(casedir,"output", subdir, paste("s", om_sim, sep=""), "ss"), overwrite = T)
  
  system(paste(file.path(casedir, "output", subdir, paste("s", om_sim, sep=""), "ss"), file.path(casedir, "output", subdir, paste("s", om_sim, sep=""), "data.ss"), sep = " "), show.output.on.console = FALSE)
  
  file.remove(file.path(casedir, "output", subdir, paste("s", om_sim, sep=""), "ss"))
...
rm(subdir, casedir, input_filename)
